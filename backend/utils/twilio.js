...
async function checkTextStatus(){
    ...
    for (let user in users){
        ...
        if(moment(dateTillBleed).diff(todaysDate,'day') <= daysTill && (!isBleeding)){
...
async function sendNotification(username, number){
    client.messages.create({
        body: `Thank you ${username} for signing up for text notifications. You will get a notfication ${daysTill} days before your period starts. You can turn them off anytime in settings.`,
        from: process.env.TWILIO_NUMBER,
        to: `${number}`
    })
}
